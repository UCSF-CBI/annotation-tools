...
REQUIRED_HEADERS = {"Content-Type" : "application/x-www-form-urlencoded", "accept" : "application/json"}
    ...
    with open(input_maf) as f1, open(output_maf, "w") as f2:
        for line in f1.readlines():
            ...
            # write out record with gnomAD values filled in to new MAF
                ...
                record = line.strip("\n").split('\t')
                genomic_variant = [
                        record[header_indices["Chromosome"]],
                        record[header_indices["Start_Position"]],
                        record[header_indices["End_Position"]],
                        record[header_indices["Reference_Allele"]],
                        record[header_indices["Tumor_Seq_Allele1"]],
                        record[header_indices["Tumor_Seq_Allele2"]]
                ]
                query = vnc.genomic_variant_to_hgvs(genomic_variant, "Homo sapiens")
...
def get_query(query):
    ...
    """
    my_variant_info_response = requests.get("%s/%s?fields=%s" % (MVI_VARIANT_ENDPOINT, "chr" + query, MVI_VARIANT_ENDPOINT_FIELDS_PARAM))
